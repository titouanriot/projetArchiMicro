...
class GroupService:
    ...
    def get_groups_from_member(self, db : Session, user: User):
        ...
        items = db.query(GroupSchema).join(BelongTo, GroupSchema.owner == BelongTo.id_group)\
    ...
    def add_user_to_group(self, group_name: str, email: str, db: Session, user: User):
        ...
        try:
            if self.checkIfExists(group_name, db) :
                ...
                if db.query(exists().where((BelongTo.id_group == new_belong_to.id_group) and (BelongTo.id_user == new_belong_to.id_user))).scalar():
